class EventsDatatable
  ...
  def initialize(view)
    @view = view
    @events = Event.where('end_time >= ?', DateTime.now).where(search_string, search: "%#{params[:sSearch] == nil ? params[:sSearch] : params[:sSearch].downcase}%").upcoming_first#.past_events
